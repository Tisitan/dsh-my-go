...
import { AGENT_TYPES, PRICE_KEY_PATTERN, ROLE_KEY_PATTERN, RUN_CODE_TOOL } from '../preset/shared/constants.mjs'
...
// 不需要宿主 /api 那套 300 MiB 附件额度；32 MiB 相对最坏的一次 saveSettings（整张
// 单价表 + 全部 persona 草稿）仍是两个数量级的余量，纯防御而从不参与正常路径。
...
export async function apply(ctx, config = {}) {
  ...
  // Write-path sanitization for one usagePrices row (D1's "reject at write
  // time"): schemastery cannot see NaN/Infinity (NaN < 0 is false), so the
  // save path is the last fence before an atomic mutate — numeric strings
  // coerce, anything not finite non-negative (or missing a required bucket)
  // nulls the row out. Lives at apply scope (not inside the register try):
  // the saveSettings RPC closure references it, and a block-scoped const
  // there would make every row-bearing save die as settings-rejected.
  // Mirrors src/usage-price-rows.js sanitizePriceRow.
  const sanitizePriceValue = (row) => {
    if (!row || typeof row !== 'object' || Array.isArray(row)) return null
    const bucket = (v) => {
      const num = typeof v === 'number' ? v : typeof v === 'string' && v.trim() !== '' ? Number(v) : NaN
      return Number.isFinite(num) && num >= 0 ? num : null
    }
    const input = bucket(row.input)
    const output = bucket(row.output)
    if (input === null || output === null) return null
    const out = { input, output }
    for (const key of ['cacheRead', 'cacheWrite']) {
      const num = bucket(row[key])
  ...
  const usageAggregator = createUsageAggregator({
    ...
    getUsagePrices: () => settings?.get?.('dsh-my-go')?.usagePrices,
    getUsageCurrency: () => settings?.get?.('dsh-my-go')?.usageCurrency,
  })
  ...
  //   | "getBuiltinPersona" | "loadSettings" | "saveSettings" | "getUsage"
    ...
    const dispatchPanelEndpoint = async (endpoint, payload) => {
      ...
      if (endpoint === 'saveSettings') {
        ...
        try {
          ...
          // usagePrices (contract D1): row-level merge write-back — a draft
          // carrying the key is the authoritative full view: provided rows
          // set (sanitized), stored rows missing from the draft unset; a
          // draft without the key touches nothing (same explicit-carry rule
          // as every field above). Dirty keys / invalid rows drop fail-closed
          // (E7/B-05): one bad row must not poison the whole atomic mutate.
          if (draft.usagePrices !== undefined && draft.usagePrices !== null && typeof draft.usagePrices === 'object' && !Array.isArray(draft.usagePrices)) {
            for (const [key, row] of Object.entries(draft.usagePrices)) {
              if (typeof key !== 'string' || !PRICE_KEY_PATTERN.test(key)) continue
              const price = sanitizePriceValue(row)
              if (price === null) {
                console.warn(`[dsh-my-go] usagePrices row dropped (invalid value): ${key}`)
                continue
              }
            ...
            try {
              const current = settingsService.get('dsh-my-go')
              const storedPrices = current && typeof current === 'object' ? current.usagePrices : undefined
              if (storedPrices && typeof storedPrices === 'object' && !Array.isArray(storedPrices)) {
                for (const key of Object.keys(storedPrices)) {
                  if (!(key in draft.usagePrices)) ops.push({ op: 'unset', path: ['usagePrices', key] })
                }
              }
            } catch { /* settings read failure: skip the deletion pass */ }
          }
          // usageCurrency (D1a): same explicit-carry discipline as every field
          // above — absent key touches nothing, illegal values drop fail-closed
          // so a hand-rolled client can never poison the atomic mutate.
          if (draft.usageCurrency !== undefined && draft.usageCurrency !== null) {
            if (draft.usageCurrency === 'USD' || draft.usageCurrency === 'CNY') {
              ops.push({ op: 'set', path: ['usageCurrency'], value: draft.usageCurrency })
            }
          }

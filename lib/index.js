...
export async function apply(ctx, config = {}) {
  ...
  ctx.inject(['connection'], (webContext) => {
    ...
    rpc.handle('dsh-my-go/snapshot', async (endpoint) => {
      if (endpoint === 'get') return { ok: true, value: latestSnapshot ?? { seq: 0, current: null, queue: [], helpRequests: [], history: [] } }
      return { ok: false, error: { code: 'bad-request', message: `unknown endpoint: ${endpoint}` } }
    }, { authority: 'trusted-host' })

    rpc.handle('dsh-my-go/listModels', async (endpoint) => {
      if (endpoint !== 'get') return { ok: false, error: { code: 'bad-request', message: `unknown endpoint: ${endpoint}` } }

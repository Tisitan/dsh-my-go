...
export async function apply(ctx, config = {}) {
  ...
  if (harness && typeof harness.handle === 'function') {
    ...
    disposers.push(harness.handle('dsh-my-go/listModels', async () => {
      const llm = ctx.get('llm')
      if (!llm) return { providers: [], models: {} }
      const providers = await llm.listProviders()
      const result = { providers: providers.map((p) => p.id), models: {} }
      for (const pid of result.providers) {
        try {
          const models = await llm.listModels(pid)
          result.models[pid] = models.map((m) => m.id)

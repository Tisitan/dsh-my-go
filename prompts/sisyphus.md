...
| 架构调试、跨模块分析、深层 Bug 定位（**仅在其他工种无法胜任时**） | `oracle` |
...
- **Oracle 是最后手段**：只有在其他任何工种（explore/librarian/looker/hermes/
  hephaestus）都明确无法胜任时，才调用 oracle。「审查 / 验证 / 核验 / 验收」报告
  不是 Oracle 的默认触发词——报告核验是 Sisyphus 质检本职，不因它启动 Oracle。
...
6. **能省则省，先派最便宜的工种**：解决问题的能力排序是
   ...
   搞不定的（跨模块/深层 Bug/终验）才上 `oracle`。
   口诀：**「任务能写多具体，工种就选多便宜；能从 Hermes 干的，别让 Hephaestus 烧钱。」**
   **Oracle 是压舱石，不是常规选项**：它只在其他所有工种都无胜任时出马；报告
   核验、例行审查、常规验收一律由 Sisyphus 质检抓实，不上 Oracle。
...
> **不存在「抽查」**：Sisyphus 不抽查任何实际产物，Oracle 的终验也**不是**抽查
> 子代理报告——Oracle 只在其他工种无法胜任时被调用做实质工作，报告核验永远是
> Sisyphus 查文本逻辑的本职。
...
| `oracle` | **最后手段**：架构调试 + 终验，仅当其他所有工种无法胜任的跨模块分析/深层 Bug | 调试/深层Bug（其他工种都无力时） |

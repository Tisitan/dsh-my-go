...
| 架构调试、跨模块分析、深层 Bug 定位、终验/验收 | `oracle` |

**任何「抽查/验证/补全/确认」冲动，凡是涉及上述操作的，一律转成正向的派发动作**（`go_work` / `continue` / `forward`），绝不自己 read/grep/写代码/分析。
...
5. **失败**：结论是失败态（✗）时，判断是重试（continue）、换工种（go_work）还是升级（oracle）。

> 注意：质检是「看输出有没有明显的逻辑问题」，**不是**逐条核验事实——那等于
> 变相自己干活。确需核实某代码/路径是否存在时，可派 `explore` 核实；但这是
> 例外动作，不是质检的必需步骤。

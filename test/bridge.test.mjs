...
test('broker.apply publishes Symbol.for("dsh-my-go.snapshot") accessor', async () => {
  ...
  await broker.apply(mockCtx(), { reportExternalization: false })
...
test('queued dispatch failure requeues and the retry timer drains the queue', async () => {
  ...
  await waitFor(() => currentOf('parent-1')?.status === 'running', { what: '队列第二发补位上机' })
  const snap = snapOf('parent-1')
  assert.equal(snap.queue.length, 0)
  assert.equal(snap.currentRecords?.[0]?.agentType, 'hermes')
  assert.equal(snap.currentRecords?.[0]?.status, 'running')
})
...
test('queued dispatch abandoned after retry cap: failed history + console.error', async () => {
  ...
  try {
    ...
    await waitFor(
      () => currentOf('parent-gone') === null && snapOf('parent-gone')?.history?.some((h) => h.status === 'failed'),
      { what: '重试上限后弃单落账' },
    )
    const snap = snapOf('parent-gone')
    assert.equal(snap.queue.length, 0)
    assert.equal(snap.currentRecords?.length ?? 0, 0)
...
test('agent/disposed before subagent/end (production order) still lands the conclusion', async () => {
  ...
  try {
    ...
    await broker.apply(ctx, { reportExternalization: false, queueRetryBaseMs: 5, disposeEndGraceMs: 50 })
    const goWork = tools.get('go_work')
    ...
    assert.equal(snap.currentRecords?.[0]?.agentType, 'hermes')
    assert.equal(snap.currentRecords?.[0]?.status, 'spawning')
    ...
    await waitFor(() => currentOf('parent-1')?.status === 'running', { what: '换键后 hermes 记录转 running' })
    assert.equal(currentOf('parent-1')?.status, 'running')
    // 宽限期兜底定时器已被 end 取消：grace 过后 explore 记录不被二次动刀
    await new Promise((r) => setTimeout(r, 100))
    const after = snapOf('parent-1')
    assert.equal(after.history.length, 1)
    assert.equal(after.currentRecords?.[0]?.agentType, 'hermes')
  } finally {
...
test('agent/disposed without subagent/end: grace fallback frees the slot and drains the queue', async () => {
  ...
  await broker.apply(ctx, { reportExternalization: false, queueRetryBaseMs: 5, disposeEndGraceMs: 10 })
  const goWork = tools.get('go_work')
  ...
  await waitFor(() => currentOf('parent-1')?.agentType === 'hermes'
    && currentOf('parent-1')?.status === 'running'
...
test('queued dispatch binds the real childId and notifies the parent with the work-id mapping', async () => {
  ...
  dispatch('subagent/end', { id: 'sess-1', stopReason: 'completed', lastAssistantMessage: [{ type: 'text', text: 'done-1' }] })
  await waitFor(() => currentOf('parent-1')?.childId === 'sess-2', { what: '队首任务上岗并 resolve 占槽' })
  assert.equal(currentOf('parent-1')?.childId, 'sess-2')
...
test('truncation config applies to orchestration_status; failed conclusions are never truncated', async () => {
  ...
  await broker.apply(ctx, { reportExternalization: false, queueRetryBaseMs: 5, statusConclusionMax: 10 })
  const goWork = tools.get('go_work')
...
test('orchestration ledger persists history to disk and reloads it on the next apply', async () => {
  ...
  try {
    ...
    await broker.apply(first.ctx, { reportExternalization: false, queueRetryBaseMs: 5 })
    ...
    // v3 落盘形状（三期 3.3，D12）：{ version: 3, parents: {...}, chains?: {...} }——
    // 无链会话省略 chains 键；v2 档由 loadLedger 兼容读（parents 恢复、chains 空起步）
    ...
    await broker.apply(second.ctx, { reportExternalization: false, queueRetryBaseMs: 5 })
...
test('台账文件兜底查找：内存全实例未命中时 continue 回读台账文件命中（现场-Z3）', async () => {
  ...
  try {
    ...
    assert.equal(snap.currentRecords?.[0]?.childId, 'sess-z3')
    assert.equal(snap.currentRecords?.[0]?.status, 'running')
...
test('step-3 a: 链首 404 终局自动切 fallbacks[0]（生产时序 disposed→end），历史带 [备选 n/m] 标注，新 child 归属原 orchestration', async () => {
  ...
  await waitFor(() => currentOf('parent-1')?.childId === 'sess-2' && snapOf('parent-1')?.history?.length >= 1, { what: '链首落史且重派新 child 已换键占槽' })
  const snap = snapOf('parent-1')
  ...
  assert.equal(snap.currentRecords?.[0]?.childId, 'sess-2')
  assert.equal(snap.currentRecords?.[0]?.agentType, 'hermes')
  assert.equal(snap.currentRecords?.[0]?.status, 'running')
  assert.equal(snap.currentRecords?.[0]?.fallbackAttempt, 1, 'fallbackAttempt 跨重派传递到新记录')
  ...
  // 兜底没误伤它」），只能用固定 sleep
  await drain(60)
  assert.equal(currentOf('parent-1')?.childId, 'sess-2', 'dispose 宽限兜底不得误伤重派新 child')
  ...
  assert.equal(after.currentRecords?.length ?? 0, 0)
})
...
test('step-3 b: 链尽（备选也失败）落失败历史，附因保留，不再重派', async () => {
  ...
  dispatch('subagent/end', { id: 'sess-1', stopReason: 'error', lastAssistantMessage: [] })
  await waitFor(() => currentOf('parent-1')?.childId === 'sess-2', { what: '备选儿童已 resolve 并占槽' })
...
test('step-3 f: once-guard——评估窗口内双发 end 不提前落史、不弃重派（棒2-Z1）', async () => {
  ...
  try {
    ...
    // 评估恢复：重派必须完成，而不是弃派
    releaseModels([{ id: 'm1' }])
    await waitFor(() => currentOf('parent-1')?.childId === 'sess-2', { what: '重派完成（修复前 specs 钉死在 1 = 静默弃派）' })
    ...
    assert.equal(snap.currentRecords?.[0]?.childId, 'sess-2', '新 child 占槽运行')
    ...
    await waitFor(() => currentOf('parent-1') === null, { what: '新 child 完工后槽位释放' })
    assert.equal(currentOf('parent-1'), null)
  } finally {
    console.warn = origWarn
  }
})
...
test('step-3 g: 墓碑共存——重派后旧 child 迟到的 agent/disposed 不误伤新 child 记录', async () => {
  ...
  await tools.get('go_work').execute({ agent: 'hermes', prompt: 'x' }, execOf(parent))
  dispatch('subagent/end', { id: 'sess-1', stopReason: 'error', lastAssistantMessage: [] })
  await waitFor(() => currentOf('parent-1')?.childId === 'sess-2', { what: '重派新 child 已占槽' })
  assert.equal(currentOf('parent-1')?.childId, 'sess-2')
  // 旧 child 的 disposed 事件在重派完成后才到达（迟到 disposed）
  dispatch('agent/disposed', { agent: { id: 'sess-1' } })
  await drain(40) // 越过 disposeEndGraceMs：若误挂 timer 此刻已触发
  assert.equal(currentOf('parent-1')?.childId, 'sess-2', '旧 child 的 disposed 不 abort 新 child 记录')
...
test('step-3 h: 多会话隔离——A 会话的备选重派归属 A，B 会话流水线不受扰', async () => {
  ...
  await waitFor(() => currentOf('parent-A')?.childId === 'sess-A2', { what: 'A 的重派已换键并占住自己的槽位' })
  const snapA = snapOf('parent-A')
  const snapB = snapOf('parent-B')
  assert.equal(snapA.currentRecords?.[0]?.childId, 'sess-A2', '重派新 child 归属 A 的流水线')
  assert.equal(snapA.currentRecords?.[0]?.agentType, 'hermes')
  assert.equal(snapB.currentRecords?.[0]?.childId, 'sess-B1', 'B 会话在跑任务不受 A 重派影响')
...
test('0.2.3-tisitan.17 a: 重派记录携带 fallbackEntry，台账 v2 round-trip 后仍在', async () => {
  ...
  try {
    ...
    await broker.apply(first.ctx, { reportExternalization: false,
      queueRetryBaseMs: 5,
      bindings: { hermes: { provider: 'p0', model: 'm0', fallbacks: [{ provider: 'p1', model: 'm1' }] } },
    })
    ...
    await waitFor(() => currentOf('parent-1')?.childId === 'sess-2', { what: '重派新 child 已占槽并携带备选条目' })
...
test('urgency=abort 护航：被掐轮的 aborted end 不落史不推进队列，续轮 end 正常收尾', async () => {
  ...
  dispatch('subagent/end', { id: 'sess-1', stopReason: 'aborted', lastAssistantMessage: [] })
  const snap = snapOf('parent-1')
  assert.equal(snap.currentRecords?.[0]?.childId, 'sess-1', '记录不落史，续轮仍占槽')
  assert.equal(snap.currentRecords?.[0]?.status, 'running')
...
test('urgency=abort 投递失败补偿：followup 抛错撤销护航，aborted end 正常落史释放槽位', async () => {
  ...
  dispatch('subagent/end', { id: 'sess-1', stopReason: 'aborted', lastAssistantMessage: [] })
  const snap = snapOf('parent-1')
  assert.equal(snap?.currentRecords?.length ?? 0, 0, '记录正常落史，槽位释放')
  assert.equal(snap?.history.length, 1)
  assert.equal(snap?.history[0]?.status, 'failed')
})
...
test('非 running 给 steer：waiting 子代理按 queued 投递并 resume，steer 零调用', async () => {
  ...
  try {
    ...
    assert.deepEqual(followups, ['sess-1'])
    assert.equal(currentOf('parent-1')?.status, 'running', 'followup 路径自带 resume 状态迁移')
...
test('urgency=abort 活体门槛：儿童不在活注册表 → 不掐断也不登记护航，降级 queued 文案（N6）', async () => {
  ...
  try {
    ...
    const snap = snapOf('parent-1')
    assert.equal(snap?.currentRecords?.length ?? 0, 0, '槽位随正常完工释放')
    assert.equal(snap?.history.length, 1)
...
test('abort 护航只吞非 completed 终局：掐断与完工赛跑时 completed end 照常落账、护航不留残（N6）', async () => {
  ...
  try {
    ...
    const snap = snapOf('parent-1')
    assert.equal(snap?.currentRecords?.length ?? 0, 0, 'completed end 正常收尾（修复前被 E5 吞）')
    ...
    assert.equal(currentOf('parent-1')?.agentType, 'hermes', '队列照常推进（无死槽）')
  } finally {
    console.warn = origWarn
  }
})
...
test('continue 打 spawning 占位记录：结构化拒绝，不掐断不投递也不假 accepted（N18）', async () => {
  ...
  assert.equal(currentOf('parent-1').prompt, 'scout', '台账 prompt 未被投进空气的指令改写')
...
test('need_help 工种识别走 typeOfAgent：活登记缺席时 label 兜底，agentType 不落空（N13）', async () => {
  ...
  await broker.apply(ctx, { reportExternalization: false, queueRetryBaseMs: 5, disposeEndGraceMs: 60000 })
  await tools.get('go_work').execute({ agent: 'explore', prompt: 'scout' }, execOf(parent))

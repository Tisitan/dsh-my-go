...
| **Hermes** | mimo-v2.5 | default | **快速执行**。批量替换、代码格式化、统一 imports、纯文本复制粘贴，以及**任何指令明确、步骤具体的执行活**（含被拆细后的常规实现）。**按任务难度分配：能写成零歧义具体步骤的任务优先派 Hermes，绝不因需求难而升舱。** | 替换、批量、全部改成、格式化、统一、按步骤执行 |
...
| **Oracle** | deepseek-v4-pro | max | **最后手段**：架构调试 + 终验。仅当其他所有工种无法胜任时的跨模块依赖分析、深层 Bug 定位、代码审查、最终验收。 | 调试、深层Bug（其他工种都无力时） |
